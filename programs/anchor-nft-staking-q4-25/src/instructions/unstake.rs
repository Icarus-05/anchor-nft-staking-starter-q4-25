...
impl<'info> Unstake<'info> {
    ...
    pub fn unstake(&mut self) -> Result<()> {
        ...
        let time_elapsed = (Clock::get()?.unix_timestamp - self.stake_account.staked_at).checked_div(86400).unwrap() as u32;

        require!(time_elapsed >= self.config.freeze_period, StakeError::FreezePeriodNotPassed);

        let points_earned: u32 = time_elapsed * self.config.points_per_stake as u32;
